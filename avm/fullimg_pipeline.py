...
class AVM:
    def __init__(self, args):
        ...
        self.aligned_info = self._align_resolution_info(H_f, W_f)
        ...
        if generate_input_imgs:
            ...
            self._generate_aligned_imgs()


    def _align_resolution_info(self, H_frame, W_frame):
        '''
        有两个输入，供外部调用用
        '''
        if W_frame % 2 == 1:
            tmp = int((W_frame + 1) / 2 * 3)
        else:
            tmp = int(W_frame / 2 * 3)
        if tmp >= H_frame:
            pad_H = tmp - H_frame
            ...
            pad_H = 6 - H_frame % 6 # %3 & %2
            pad_W = int((H_frame + pad_H - tmp) / 3 * 2)
        pad_left = pad_right = int(pad_W / 2)
        if pad_W % 2:
            pad_left += 1
            pad_W += 1
    ...
    def _align_img(self, image, aligned_info):
        '''
        有两个输入，供外部调用用
        '''
        # 获取384:128=3:1比例对齐信息
    ...
    def _generate_aligned_imgs(self):
        # 对齐图片
        ...
        for file in os.listdir(self.src_frames_dir):
            ...
            # 对齐比例，获得左右子图
            img_left, img_right = self._align_img(image, self.aligned_info)
            # 命名左右子图
            name_left = 'l_' + file
        ...
        print('finished _generate_aligned_imgs')
        return
    ...
    def gengerate_single_input(self, params, image):
        """ 外部函数 """
        # 获取比例对齐信息
        H_f, W_f, _ = image.shape
        aligned_info = self._align_resolution_info(H_f, W_f)
        # 比例对齐
        img_left, img_right = self._align_img(image, aligned_info)
        # resize到model输入分辨率
        res = [aligned_info]
